...
> **Status:** Actively in development. Current release covers the full pipeline: data ingestion, correlation, RAG retrieval, dual-model reasoning with cross-verification, and a gated autonomous action layer that opens real incident-report PRs when confidence is high enough. ChatOps delivery is in progress — see [Roadmap](#roadmap).
